...
export default function DeleteTattooModal({deleteRowId, setOpenDelete, openDelete}) {

    const deleteTattoo = async (id) => {
        const q = query(collection(db, "tattooGallery"), where("id", "==", id));
    ...
    <div className='w-full h-full absolute bg-black/30 top-0 z-50'>
        <div className='flex flex-col mx-auto  items-center gap-14 w-[45%] h-auto py-10 mt-36 bg-[#3b3b3b] text-white rounded-xl'>
            ...
            <div className='flex items-center justify-center gap-7'>
                <button className='bg-white text-black px-7 py-2 rounded-xl hover:bg-black hover:text-white duration-100' onClick={() => {deleteTattoo(deleteRowId); setOpenDelete(false)}}>Confirm</button>
